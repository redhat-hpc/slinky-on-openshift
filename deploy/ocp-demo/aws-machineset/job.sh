...
ocp_aws_create_machineset(){
  ...
  MACHINE_SET_TYPE=$(oc -n openshift-machine-api get machinesets.machine.openshift.io -o name | grep "storage" | head -n1)
  ...
  oc -n openshift-machine-api \
    ...
    --type=merge --patch '{"spec":{"template":{"spec":{"metadata":{"labels":{"node-role.kubernetes.io/storage":""}}}}}}'
...
ocp_aws_clone_machineset(){
  [ -z "${1}" ] && \
  echo "
    usage: ocp_aws_create_machineset < instance type, default g4dn.4xlarge >
  "
